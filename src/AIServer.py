...
#from defDF import compareFace
#from lastGVapi import image_analysis
import csv
import threading
...
@app.route('/upload_add', methods=['POST'])
#@synchronized(lock)
def upload_image():
...
@app.route('/upload_delete', methods=['POST'])
#@synchronized(lock)
...
def process_circles(db_link,save_path, circles_data):
    ...
    for circle in circles_data:
        object = detect_and_draw_objects_in_radius(save_path, circle['centerX'], circle['centerY'], circle['radius'],db_link)
        if object:
